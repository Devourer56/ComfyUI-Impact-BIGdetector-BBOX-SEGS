...
Colored preview: Stage 1 = Red, Stage 2 = Green, Stage 3 = Blue, Combined = Yellow."""
    ...
    def process(self, image: torch.Tensor, mode: str, target_size: int, max_size: int, iou_threshold: float, include_masks_in_output: bool, simplify_masks: bool, simplify_kernel_size: int, simplify_iterations: int, segs_input=None, stage_1_enabled=True, stage_1_detector_type="bbox", stage_1_bbox_detector=None, stage_1_segm_detector=None, stage_1_confidence=0.25, stage_1_iou_threshold=0.45, stage_1_dilation=0, stage_1_classes="", stage_1_crop_factor=3.0, stage_1_scale_mode="bbox", stage_1_target_size=640, stage_1_max_size=1024, stage_1_process_empty=False, min_confidence=0.0, min_bbox_width=1, min_bbox_height=1, stage_2_enabled=True, stage_2_detector_type="bbox", stage_2_bbox_detector=None, stage_2_segm_detector=None, stage_2_confidence=0.25, stage_2_iou_threshold=0.45, stage_2_dilation=0, stage_2_classes="", stage_2_crop_factor=3.0, stage_2_scale_mode="bbox", stage_2_target_size=640, stage_2_max_size=1024, stage_2_process_empty=False, stage_3_enabled=True, stage_3_detector_type="bbox", stage_3_bbox_detector=None, stage_3_segm_detector=None, stage_3_confidence=0.25, stage_3_iou_threshold=0.45, stage_3_dilation=0, stage_3_classes="", stage_3_crop_factor=3.0, stage_3_scale_mode="bbox", stage_3_target_size=640, stage_3_max_size=1024, stage_3_process_empty=False, drop_size=1, extra_pnginfo=None, prompt=None):
        logger.info(f"Starting Cascade Detector (FIXED) in mode: {mode}")
        if not self.IMPACT_AVAILABLE:
        ...
        if mode == "sequential":
            results, stage_assignments = self.process_sequential(image, initial_segs, enabled_stages, detectors_info, settings, include_masks_in_output, original_shape_hw, original_image_np, simplify_masks, simplify_kernel_size, simplify_iterations, stage_1_process_empty, stage_2_process_empty, stage_3_process_empty)
        else:
            results, stage_assignments = self.process_parallel(image, initial_segs, enabled_stages, detectors_info, settings, include_masks_in_output, original_shape_hw, original_image_np, simplify_masks, simplify_kernel_size, simplify_iterations, stage_1_process_empty, stage_2_process_empty, stage_3_process_empty)
...
NODE_DISPLAY_NAME_MAPPINGS = {"CascadeDetectorAdvanced": "🎯 Cascade Detector Advanced (FIXED)"}
